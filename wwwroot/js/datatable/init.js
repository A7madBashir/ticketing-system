...
(function (window, $) {
  ...
  window.initializeDataTable = function (options) {
    if (!options || !options.selector || !options.columns) {
      console.error(
        "DataTableInitializer: Missing required options (selector, columns)."
      );
      return null;
    }
    ...
    if (!$table.length) {
      console.warn(
        `DataTableInitializer: Table with selector "${options.selector}" not found.`
      );
      return null;
    }
    ...
    const defaultDataTableOptions = {
      processing: true,
      serverSide: false, // Assume client-side unless AJAX is provided
      ...
      // Other common defaults can go here
    };

    // Deep merge user-provided DataTables options with defaults
    const dataTableConfig = $.extend(
      true,
      {},
      ...
      options.dataTableOptions,
      {
        ...
        data: options.data, // Will be undefined if not provided
      }
    );

    // Handle AJAX configuration separately to correctly set `serverSide` and `ajax.data` function
    if (options.ajax) {
      dataTableConfig.serverSide = true;

      // Clone the ajax object to avoid modifying the original options object
      dataTableConfig.ajax = $.extend(true, {}, options.ajax);

      // If a custom 'data' function is provided in options.ajax, use it.
      // Otherwise, DataTables will use its default data serialization.
      // This is the place for custom search parameters.
      if (options.ajax.data && typeof options.ajax.data === "function") {
        // The provided function will be called by DataTables,
        // receiving the default request object 'd'.
        // We simply assign it.
        dataTableConfig.ajax.data = options.ajax.data;
      } else if (options.ajax.data && typeof options.ajax.data === "object") {
        // If it's a plain object, wrap it in a function to merge with DataTables' own params
        const staticCustomParams = $.extend(true, {}, options.ajax.data); // Clone to prevent mutation
        dataTableConfig.ajax.data = function (d) {
          return $.extend(d, staticCustomParams);
        };
      }
      // If options.ajax.data is undefined, DataTables will send its default parameters.
    }

    // Initialize the DataTable
    const dataTableApi = $table.DataTable(dataTableConfig);

    // Bind events if provided
    if (options.events && typeof options.events === "object") {
      for (const eventName in options.events) {
        if (
          Object.prototype.hasOwnProperty.call(options.events, eventName) &&
          typeof options.events[eventName] === "function"
        ) {
          // DataTables events are typically bound using the .on() method
          // The event name might need to be prefixed with '.dt' for some events
          // or for global events, but simple string names often work directly.
          // For example: 'init.dt', 'draw.dt', 'click' (on row/cell)
          $table.on(eventName, options.events[eventName]);
        }
      }
    }
